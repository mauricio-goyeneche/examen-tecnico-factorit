...
@section('title', 'Crear Alumno - FactorIT Proyecto Técnico')

@section('content')
    <div class="row bg-light p-3">
        <p class="fs-2 text-center mb-4 border-bottom border-dark">Crear Alumno</p>
        ...
        <div class="col-12">
            <form action="{{ route('alumnos.store') }}" method="post" class="col-6 m-auto">
                @csrf
                    ...
                    @if (!$asignaturas->isEmpty())
                        @foreach ($asignaturas as $asignatura)
                            <div class="form-check">
                                ...
                                <input class="form-check-input" type="checkbox" id="{{ $asignatura->descripcion }}"
                                    value="{{ $asignatura->id }}" name="asignaturas[]">
                            </div>
                        @endforeach
                    @else
                        <div class="alert alert-info alert-dismissible fade show text-center" role="alert">
                            No hay asignaturas para inscribirse.
                        </div>
                    @endif
                </div>

                <div class="mt-2 text-center">
                    <button class="btn btn-info" type="submit">Guardar alumno</button>
